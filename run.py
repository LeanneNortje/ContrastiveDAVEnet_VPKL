...
def validate(audio_model, image_model, attention, contrastive_loss, val_loader, rank, args):
    ...
    with torch.no_grad():
        for value_dict in tqdm(val_loader, leave=False):
            ...
            s, a = attention.encode(image_output, english_output, english_nframes)
            ...
            for p, pos_dict in enumerate(value_dict['positives']):
                ...
                score, a = attention.encode(image_output, pos_english_output, pos_english_nframes)
                positives.append(score)

                score, a = attention.encode(pos_image_output, english_output, english_nframes)
                positives.append(score)
            ...
            for n, neg_dict in enumerate(value_dict['negatives']):
                ...
                score, a = attention.encode(image_output, neg_english_output, neg_english_nframes)
                negatives.append(score) 

                score, a = attention.encode(neg_image_output, english_output, english_nframes)
                negatives.append(score)
